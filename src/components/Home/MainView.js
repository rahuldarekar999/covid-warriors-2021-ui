                ...
                <code>
                  {(allGenLink && allGenLink.length > 0) &&
                    allGenLink.map((mo, index) => {
                      return (<div key={mo.mobile}><a style={{ color: '#007bff', cursor: 'pointer' }} onClick={() => { redirectLink(mo, index) }}>{mo.mob}</a> &nbsp;&nbsp;<a style={{ color: '#007bff', cursor: 'pointer' }} onClick={() => { redirectLink(mo, index) }}>send</a>{mo.isClicked && <FontAwesomeIcon className={''} icon={faCheck} style={{ color: "#00b526", marginLeft: 3 }} />}</div>);
                    })
                  }
                </code>
              </div>}
