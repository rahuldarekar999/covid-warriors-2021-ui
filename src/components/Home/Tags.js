  ...
  if (tags) {
    return (
      <div className="tag-list">
        ...
        <a
          href=""
          className={`tag-default tag-pill ml-2 sendMessageBtn`}
          key={'refresh'}

          onClick={(ev)=>{
              ev.preventDefault();
              props.onClickTag(props.tag, props.city, page => agent.Articles.byTag(props.tag, props.city.value, page), agent.Articles.byTag(props.tag, props.city.value));
          }}
        ><FontAwesomeIcon icon={faSyncAlt} style={{ color: '#ffffff' }} /></a>
